...
Language (aka BISL) proposed by the CAT
...
decide}} and implemented in the \textsc{Frama-C} platform. As its name
suggests it, it aims at specifying behavioral properties of C source
code. The main inspiration for this language comes from the
...
supposed to be necessary for a basic use of that specification language.
...
\item[left-values] \index{left-value}\index{lvalue} 
  ...
  an assignment, e.g., a non-constant memory location.
...
\item[function behavior] \index{function behavior} \index{behavior}
  ...
  possibly restricted set of calls to that function.
...
written directly in C source files, so
