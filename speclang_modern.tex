...
\lstinline|loop allocates| and \lstinline|loop frees| have the same semantics.
\begin{listing-nonumber}
...
before entering the loop. Namely, as for \lstinline|loop assigns|, 
\lstinline|loop allocates| defines a loop invariant.

More precisely, with \lstinline|L$_0$| labelling the state before entering
the loop, the clause \lstinline|loop allocates Q$_1$,$\ldots$,Q$_n$|
