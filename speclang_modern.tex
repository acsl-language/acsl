...
All of them takes optional label identifier as first argument. 
The default value of that label is defined in Section~\label{sec:default-logic-labels}.
\begin{itemize}
\item \lstinline|\baseaddr{L}(p)|\indexttbs{base\_addr}
returns the base address of the allocated block
containing, at the label \lstinline|L|, the pointer \lstinline|p|
\begin{listing-nonumber}
\base_addr{id} : $\alpha$ * $\ra$ char*
\end{listing-nonumber}
\item \lstinline|\blocklength{L}(p)|\indexttbs{block\_length}
  returns the length (in bytes) of the allocated block containing, 
  at the label \lstinline|L|, its argument pointer.
\begin{listing-nonumber}
\block_length{id} : $\alpha$ * $\ra$ size_t
\end{listing-nonumber}
\end{itemize}
...
\begin{listing-nonumber}
\valid_read{id} : set<$\alpha$ *> $\ra$ boolean
\end{listing-nonumber}
\end{itemize}

\lstinline|\valid{L}(p)| implies \lstinline|\valid_read{L}(p)| but the reverse is 
...
type of its argument. Namely, \lstinline|\valid{L}((int *) p)| and 
...
\lstinline|s| and label \lstinline|L|, \lstinline|\valid{L}(s)| if and only if for all \lstinline|p$\in$s|:
  \begin{listing-nonumber}
