...
\label{chap:base}
\newcommand{\ifCPP}[1]{\ifthenelse{\isCPP}{\input{#1}}{}}
...
Note that the semantics of these constructs implies that in concept two different executions of the 
program take place in parallel: one using the numeric precision as given in the program and one using mathematical real arithmetic. $\verb|\exact|(x)$ then gives the difference between the two for a specific expression and program state. If the argument to $\verb|\exact|(x)$ is just a constant or 
an expression dependent on just some local computations, then the scope of the two computations is
correspondingly constrained. But if the argument depends on program variables whose data dependencies extend back into the program, then the effect of a single $\verb|\exact|(x)$
construct is considerable. This has obvious performance implications for runtime assertion checking 
of ACSL specifications. However, even for deductive verification, if the data dependence of the argument of 
$\verb|\exact|(x)$ extends to multiple functions, the reasoning required becomes non-modular or 
requires specifications about the differences between exact and non-exact computations.
...
an \assigns{} clause must be a tset containing modifiable l-values, as described in
Section~\ref{sec:glossary}. 
More generally, we introduce syntactic constructs to denote \emph{sets of
  terms} (tsets) that are also useful for the \separated predicate (see Section~\ref{sec:separated}). The terms in a set may have any type, though the operations described below are only well-typed for certain types of tsets. For example, \lstinline|s$_1$[s$_2$]| as defined below is only well-typed if one of 
...
\item \lstinline|x \in s| holds if and only if \lstinline|x| is an element of \lstinline|s| (that is, \lstinline|s$_1$| is a subset of \lstinline|s$_2$|);
...
Similarly, for example, if \lstinline|x| is a set of memory locations but \lstinline|y| is a simple variable (say of type \lstinline|int|), then \lstinline|assigns x,y;| means
\lstinline|assigns \union(x,{y});|. If \lstinline|x| and \lstinline|y| are both simple variables, then \lstinline|assigns x,y;| means
...
\item\index{statement contract}\index{contract}
  ...
  its behavior in a similar manner to function contracts.
\end{itemize}
...
\ifCPP{cpp-class-contracts}
\ifCPP{cpp-enum}
\ifCPP{cpp-templates}
\ifCPP{cpp-types}
\ifCPP{cpp-visibility}
\ifCPP{cpp-attributes}
\ifCPP{cpp-namespaces}
...
It is a set of values of some pointer type as defined in Section~\ref{sec:locations}. As indicated below where necessary,
...
(see Section~\ref{sec:locations}) each of whose elements has some pointer type (other than \lstinline|void*|). 
...
applies to tsets (see Section~\ref{sec:locations}) of some pointer type
...
\ifthenelse{\isCPP}{\marginpar{Other C++ topics to include: templates, namespaces, class-level specs, when do invariants hold, restrictions on invariant footprint, function parameters, lambda expressions, specs for overloaded operators, specs for implicit constructors and destructors, types}}{}
