...
\begin{listing}{1}
predicate \is_nan_float`\indexttbs{is\_nan\_float}`(float x);
predicate \is_nan_double`\indexttbs{is\_nan\_double}`(double x);
predicate \is_finite_float`\indexttbs{is\_finite\_float}`(float x);
predicate \is_finite_double`\indexttbs{is\_finite\_double}`(double x);
\end{listing}
...
\item \lstinline|\old(e)|\indexttbs{old} denotes the value of
  \lstinline|e| in the pre-state of the function.
...
\item The caller of the function must guarantee that it is called in a
  state where the property \lstinline|$P_1$ && $P_2$ && `\dots`| holds.
\item The called function returns a state where the property
  \lstinline|$E_1$ && $E_2$ && `\dots`| holds.
...
% [Patrick B.] START proposition:
  ...
  invariant for the locations wich are not member of \lstinline|L|.
% END proposition:
\end{itemize}
...
\subsection{\protect\reads{} clauses}
...
(i.e. \lstinline|/*@ ghost ... */| or \lstinline|//@ ghost `dots`|
for a one-line
