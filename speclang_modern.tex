...
\begin{figure}[p]
  \fbox{\begin{minipage}{0.97\textwidth} \input{term_modern.bnf}
    \end{minipage}}
...
\item[Functional modifier] The composite element modifier is an additional
  ...
  The expression \lstinline|{ s for id = v }|
...
possible values for terms in logic. They can passed as parameters (and
also returned) to logic functions, tested for equality, etc. like any
other values.

Aggregate types can be declared in logic, and their contents may be
...
Array types in logic may be declared either with an explicit length,
non-negative. Access to the length of an logic array can be done with
\verb|\length|.
...
Functional update of a struct is done by
\begin{verbatim}
{ s for .x = 3.0 }
\end{verbatim}

There is no particular syntax for a functional update of a union: it
is equivalent to a direct initialization.
...
//@ logic integer g(struct S s) = s.t[3]; 
//@ logic struct S f(integer n) = { .x = (int)n , .y = (float)0.0 };
...
Thus, unlike in C, their is no implicit conversion from an array type
to a pointer type. However, there is an implicit conversion from array
of a given size to arrays with unspecified size (but not the
converse).

\begin{example}
  logic point[4] square = { { ... } , { .. } ... }
...
The equality predicate \verb|==| applies to aggregate values, but it
...
An explicit cast from a array type to a pointer type is allowed only
for arrays that lies in C memory, similarly as taking its address (see
...
\[
\verb|length|(t) = (\verb|block_length|(p) - \verb|offset|(p))/sizeof(\tau) 
\]
...
A particular caution should be taken care with respect to the address
operator. Values in logic do not lie in C memory so it does not mean
...
contracts. 
\begin{notreviewedenv}
Non-terminal \textsl{location} and \notimplemented{\textsl{abrupt-clauses}}
are defined later in Section~\ref{sec:locations}
and~\ref{sec:abrupt-clauses} respectivily.
Informally, \textsl{location} denotes a set of C l-values\index{l-value} 
and \textsl{abrupt-clauses} can be used to specify properties to 
functions that may exit the program (\emph{e.g.} by calling the \exit{}
function).
\end{notreviewedenv}
...
involving \emph{named behaviors}. The \lstinline|decreases| and
\lstinline|terminates| clauses are presented later in
Section~\ref{sec:termination}. 


\subsection{Built-in constructs \protect\old{} and \protect\result{}}
...
So, \lstinline|\old| construct is useless for formal parameters (in
...
\item The caller of the function must guarantee that it is called in a
  state where the property \lstinline|$P_1$ && $P_2$ && ...| holds.
\item The called function returns a state where the property
  \lstinline|$E_1$ && $E_2$ && ...| holds.
...
simplified one:
\begin{listing}{1}
/*@ requires $P_1$ && $P_2$ && `\dots`;
  @ assigns $L_1$,$L_2$,`\dots`;
  @ ensures $E_1$ && $E_2$ && `\dots`;
  @*/
\end{listing}
...
\item The caller of the function must guarantee that the call is
  ...
  \lstinline|$P$ && ($A_1$ ==> $R_1$) && ($A_2$ ==> $R_2$)| holds.
\item The called function returns a state where
the properties \lstinline|\old($A_i$) ==> $E_i$| hold for each~$i$.
...
simplified one:
\begin{listing}{1}
/*@ requires $P$ && ($A_1$ ==> $R_1$) && ($A_2$ ==> $R_2$);
  @ assigns $L_1$,$L_2$;
  @ ensures \old($A_1$) ==> $E_1$ && \old($A_2$) ==> $E_2$;
  @*/
\end{listing}
...
array \lstinline|t| to be sorted in increasing order: this is the purpose of the
predicate named \lstinline|t_is_sorted| in the \assumes\ clause of the
...
R ==> ($A_1$ || $A_2$ || ... || $A_n$)
\end{listing-nonumber}
holds, where $R$ is the precondition of the contract.
\begin{notimplementedenv}
...
in \lstinline|\separated| predicate (see Section~\ref{sec:separated}). More
...
\item \lstinline|&s| denotes the set of \lstinline|&x| for each
  \lstinline|x $\in$ s|.
\item \lstinline|$s_1$[$s_2$]| denotes the set of
\lstinline|$x_1$[$x_2$]| for each $x_1 \in s_1$
  and $x_2 \in s_2$.
\item \lstinline|$t_1$ .. $t_2$| denotes the set of integers between $t_1$ and
  $t_2$, included. If $t_1 > t_2$, this the same as \lstinline+\empty+
\item \lstinline|\union($s_1$,$\ldots$,$s_n$)|\indexttbs{union}
  denotes the union of $s_1,s_2,\ldots $ and $s_n$;
\item \lstinline|\inter($s_1$,$\ldots$,$s_n$)|\indexttbs{inter}
  denotes the intersection of $s_1,s_2,\ldots $ and $s_n$;
\item \lstinline|$s_1$+$s_2$| denotes the set of
  \lstinline|$x_1$+$x_2$| for each $x_1 \in s_1$
  and $x_2 \in s_2$;
...
\item The variant \lstinline|for $id_1$,$\ldots$,$id_k$: assert p|
...
\index{invariant}
The semantics of loop invariants is defined as follows: a simple loop
...
\item The predicate \lstinline|I| is an inductive invariant, that is
  ...
  \lstinline{continue} statement, \lstinline|I| is true in the
  resulting state. If the loop condition has side effects, these are
...
% [Patrick B.] START proposition:
  In fact, the \lstinline|loop assigns| clause specifies an inductive
  invariant for the locations that are not member of \lstinline|L|.
% END proposition:
\end{itemize}

A loop annotation preceded by
\lstinline|for id_1,`\ldots`,id_k:| is similar as above,
although applies only for behaviors
\lstinline|id_1|,$\ldots$,\lstinline|id_k|, hence in
particular holds only under the assumption of their \texttt{assumes}
clauses.
...
\subsection{Built-in construct \protect\at{}}
\label{sec:at}
...
expression \lstinline|e| in the state at label \lstinline|id|. The
label \lstinline|id| can be either a regular C label, or a label added
...
\item The label \lstinline|Here| is visible in all statement annotations,
  ...
  \lstinline|requires| and the \lstinline|assumes|,
  \lstinline|variant|,
...
\begin{todo}
  Ajouter un exemple avec un \For{}.
\end{todo}

\begin{notreviewedenv}
The \ensures\indextt{ensures} clause does not constraint the
post-state when the annoted statement terminates abruptly 
with a \Break{}, \Continue{} or \Return{} statement, 
or a call to the \exit{} function.
To specify such behaviors, \textsl{abrupt clauses} (described in 
Section~\ref{sec:abrupt-clauses}) can be used.
\end{notreviewedenv}
...
\begin{example}
  In example~\ref{ex:bsearch2}, a loop variant \lstinline|u-l| decreases
...
\begin{listing-nonumber}
\forall $y_1,\ldots,y_m$, $h_1$ ==> $\cdots$ ==> $h_l$ ==> P($t_1$,$\ldots$,$t_n$)
\end{listing-nonumber}
where \lstinline|P| occurs only positively in hypotheses $h_1,\ldots,h_l$
...
\item[Abstraction]\indexttbs{lambda}
  The term \lstinline|\lambda $\tau_1$ $x_1$,$\ldots$,$\tau_n$ $x_n$; t|
...
\item[Extended quantifiers]
  ...
  \lstinline|integer|, and $t_3$ must be a unary function with an integer
...
\subsection{Memory footprint specification: \lstinline|reads| clause}
...
\listinginput{1}{import.c} where the file \verb|list.acsl| contains logic
...
memory state:
\begin{itemize}
\item \baseaddr\indexttbs{base\_addr}
returns the base address of an allocated pointer
\begin{listing-nonumber}
\base_addr : $\alpha$ * $\ra$ char*
\end{listing-nonumber}
\item \blocklength\indexttbs{block\_length}
  returns the length of the allocated block of a pointer
\begin{listing-nonumber}
\block_length : $\alpha$ * $\ra$ size_t
\end{listing-nonumber}

\item \valid\indexttbs{valid}
  applies to a set of terms (see Section~\ref{sec:locations})
  of some pointer type. \lstinline|\valid(s)| holds if and only if
 dereferencing any $p\in \lstinline|s|$ is safe. In particular,
\lstinline|\valid(\empty)| holds.
\end{itemize}
...
\item \lstinline|\null|\indexttbs{null} is an
  ...
  the constant \lstinline|0| can have any pointer type.
% \footnote{or to \texttt{NULL}, if
%    annotations are pre-processed are the appropriate inclusion of
%    \texttt{stddef.h} is added}
\item \begin{notimplementedenv}\lstinline|\offset(p)|\end{notimplementedenv}%
  \indexttbs{offset}
  returns the offset between \lstinline|p| and its base address

  \begin{tabular}{rcl}
    \lstinline|\offset| &:& \lstinline|$\alpha$ * $\ra$ size_t| \\
    \lstinline|\offset(p)| &=& \lstinline|(char*)p - \base_addr(p)|
  \end{tabular}
  the following property holds: for any set of pointers
\lstinline|s|, \lstinline|\valid(s)| if and only if for all \lstinline|p$\in$s|:
  \begin{listing-nonumber}
...
\lstinline|\separated($loc_1$,..,$loc_n$)|\indexttbs{separated}
means that for each
...
\begin{itemize}
\item \lstinline|\fresh(p)|\indexttbs{fresh}
indicates that \lstinline|p| was not allocated in the pre-state.

\item \begin{notimplementedenv}\lstinline|\freed(p)|\end{notimplementedenv}%
\indexttbs{freed},
  indicates that \lstinline|p| was allocated in the pre-state but that it is not
...
$l_1$ of type \lstinline|set<$\tau_1$*>| and $l_2$ of type
\lstinline|set<$\tau_2$*>|)
\begin{listing}{1}
\separated($l_1$,$l_2$) <==>
  (\forall \tau_1* p; \forall \tau_2* q;
     \subset(p,$l_1$) && \subset(q,$l_2$) ==>
       (\forall \integer i,j;
         0 <= i < \sizeof($\tau_1$) && 0 <= j < \sizeof($\tau_2$) ==>
           (char*)p + i != (char*)q + j))
\end{listing}

and a clause \lstinline|assigns $l_1$,...,$l_n$| is equivalent
...
does not constraint the post-state when the annoted function and
statement terminates respectively abruptly. In such cases,
\textsl{abrupt clauses} can be used inside \textsl{simple clause} or 
\textsl{behavior body}. The allowed constructs (shown in  
Figure~\ref{fig:gram:abrupt-clauses}) differs between function and 
statement contracts.
\end{notreviewedenv}



The clauses \breaks{}, \continues{} and \returns{}
...
\assigns{} and \ensures{}, the value of
...
In such clause, \result is bound to the return code,
\emph{e.g} the value returned by \main{} or the argument passed
to \exit{}.
...
\begin{example}
  ...
  The specification of the \lstinline|may_exit| function is incomplete
  in the sense its contract allows modifications of the variable 
  \lstinline|status| when no exit is performed.
  A complete specification could be:
  \listinginput{9}{mayexit.c}
\end{example}
\end{notreviewedenv}


\begin{todo}
  Discuter de ext versus return pour la fonction main. Discuter
...
\item It is however possible to write multi-line annotations for ghost
  ...
  As in normal annotations, \verb|@|s at the beginning of a
...
\lstinline|v|
is replaced by \lstinline|g(&x,v)|.
...
\begin{notimplementedenv}\lstinline|\initialized|\end{notimplementedenv}
