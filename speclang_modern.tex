...
ACSL text is contained in C comments. Within a comment, the lexical structure mostly follows that of ANSI/ISO C. A few differences
must be noted.
\begin{itemize}
...
\item[Functional modifier]
  ...
  which returns an array whose value is the same as \lstinline|t|, except for the
...
\item \lstinline|x \in s| holds if and only if \lstinline|x| is an element of s.
