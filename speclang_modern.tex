...
% [Patrick B.] START proposition:
  ...
  \lstinline|x|, because function calls modify a copy of the effective operands,
  and the effective operands remain unaltered.  In fact, \lstinline|x| denotes
  the effective operand of \lstinline|isqrt| calls which has the same value
...
\lstinline|assigns| and \lstinline|ensures| clauses and refers to the
post-state of the function it occurs in.
\end{itemize}
...
\item A strong type invariant on type $\tau$ must hold at any step
  ...
  variable, or formal parameter with static type $\tau$. If the
