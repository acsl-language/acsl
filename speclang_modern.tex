...
in \lstinline|loop assigns| clauses.
...
hence in particular holds only under the assumption of their \texttt{assumes}
clauses.
...
\\ \makebox[5mm]{} \lstinline|\base_addr{id} : $\alpha$ * $\ra$ char*|
...
\\ \makebox[5mm]{} \lstinline|\block_length{id} : $\alpha$ * $\ra$ size_t|
\end{itemize}
...
\\ \makebox[5mm]{} \begin{tabular}{lll}
    \lstinline|\offset{id}| &:& \lstinline|$\alpha$ * $\ra$ size_t| \\
...
it is left for convenience like \assigns clauses.
...
That is the only one difference between the clauses \lstinline|allocates| and \lstinline|frees|.

Lets now define what is meaning by unmodified {\sl allocation status}:
The clause \lstinline|allocates| \lstinline|Q$_1$,$\ldots$,Q$_n$| is equivalent
...
\\ \makebox[5mm]{} \lstinline|\allocation{id} : $\alpha$ * $\ra$| 
\begin{notimplementedenv}\lstinline|{\static, \register, \auto, \dynamic, \unallocated}|
\end{notimplementedenv}%
...
\\ \makebox[5mm]{} \lstinline|\allocable{id} : $\alpha$ * $\ra$ boolean|
\\
...
\\ \makebox[5mm]{} \lstinline|\freeable{id} : $\alpha$ * $\ra$ boolean|
\\
For any pointer \lstinline|p| and label \lstinline|L|
\begin{listing-nonumber}
...
\\ \makebox[5mm]{} \lstinline|\fresh{id,id} : $\alpha$ *, \integer $\ra$ boolean|
\\
For any pointer \lstinline|p| and label \lstinline|L|
\begin{listing-nonumber}
...
\begin{example}
  ...
  Note that logic label \lstinline|Here| et \lstinline|Old| can be omitted.
\end{example}

\subsubsection{Allocation clauses for loop annotations}


\subsubsection{Default value for allocation clauses}
...
and a clause \lstinline|assigns L$_1$,$\ldots$,L$_n$| is equivalent
to the postcondition
\begin{listing-nonumber}
...
and \assigns{} clauses. On the contrary,
...
% [Patrick B.] START proposition:
Contrary to the \ensures clause, the \assigns\indextt{assigns} clause of function 
and statement contracts constrains the post-state 
