...
\end{listing-nonumber}
where the the bounds \verb|CHAR_MIN| and \verb|CHAR_MAX| are defined
in \verb|limits.h|
\end{example}
...
Unlike in C, 
there is no implicit cast from from an array type to a pointer type.
...
There is no particular syntax for functional update of an union. 
...
Thus, \old construct is useless for formal parameters (in
...
in \Loop \assigns clauses (see section~\ref{sec:loop_annot}).
...
\item When a loop exits with \lstinline{break} or \lstinline{return} or
  ...
  In such cases, locations that are not member of $L$ can be
...
\indextt{Pre}\indextt{Here}\indextt{Old}\indextt{Post}\indextt{LoopEntry}\indextt{LoopCurrent}
There are six predefined logic labels: \lstinline|Pre|, \lstinline|Here|,
\lstinline|Old|,
\lstinline|Post|, \lstinline|LoopEntry| and \lstinline|LoopCurrent|. \lstinline|\old(e)|\indexttbs{old} is
...
\item The label \begin{notimplementedenv}\lstinline|LoopEntry|\end{notimplementedenv} is visible in
all annotations related to a statement enclosed by a loop 
and refers to the state just before entering that loop.
When the annotation is a loop annotation, the label is always visible 
and refers to the state just before entering the annoted loop,
and not the enclosed loop.

\item The label \begin{notimplementedenv}\lstinline|LoopCurrent|\end{notimplementedenv} is visible in
all annotations related to a statement enclosed by a loop 
and refers to the state just before the body of that loop.
When the annotation is a loop annotation, the label is always visible 
and refers to the state just before the body of that annoted loop,
and not the enclosed loop.
Inside loop annotations, the labels \lstinline|LoopCurrent| and \lstinline|Here| are equivalent,
except inside clauses \Loop \frees (see section~\ref{sec:allocation-clause}) 
...
\emph{footprint} of an hybrid predicate or function, that is the set of
...
\label{sec:pointers} 
% [Patrick B.] START proposition:
Grammar for terms and predicates is
...
(see below) in the post-state than in the pre-state. The only one difference 
...
\begin{listing-nonumber}
\forall char* p; 
\separated(\union(Q$_1$,$\ldots$,Q$_n$),p)==>(\base_addr{Here}(p)==\base_addr{Pre}(p)
...
returns the allocation status of the block containing, at the label \lstinline|L|, the pointer \lstinline|p| 
...
before entering the loop. 
The only one difference between these two clauses is that sets \lstinline|P$_i$|are evaluated in the state before entering the loop (label \begin{notimplementedenv}\lstinline|LoopEntry|\end{notimplementedenv}), and \lstinline|Q$_i$| are evaluated in the current loop state (label \begin{notimplementedenv}\lstinline|LoopCurrent|\end{notimplementedenv}).

Namely, as for \Loop \assigns, loop annotations
\Loop \frees and \Loop \allocates define a loop invariant.

More precisely, using \lstinline|LoopEntry|\indextt{LoopEntry} labelling the state before entering
the loop, the loop annotation
\begin{listing-nonumber}
...
\begin{example}
The addresses of locations \lstinline|q[0..n]| are not modified by the loop,\indextt{LoopEntry}\indextt{LoopCurrent} 
 \listinginput{1}{loop-frees.c}
but their values are.
The clause \Loop \frees catches the set of the memory blocks that can be released by the previous loop iterations.
The first \Loop \invariant defines exactly these memory blocks
while the second defines the memory blocks to be released by next iterations.
\end{example}

A {\sl loop-clause} without allocation clause implicitly contents \Loop \allocates \nothing.
...
\begin{example}
  ...
  In the first definition, since union is made with a
...
\begin{example}
  ...
  clause of a statement contracts.
...
\indexttbs{specified}
\begin{notimplementedenv}\lstinline|\specified{L}|\end{notimplementedenv}
...
argument and means that each l-value in this set has a \emph{specified
  value} at label \lstinline|L|: its value is not a dangling pointer (that is, the value
is not the address of a local variable refered to outside of its scope).
\\ \makebox[5mm]{} \lstinline|\specified{id} : set<$\alpha$*> $\ra$ bool|
