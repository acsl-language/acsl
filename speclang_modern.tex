...
\indextt{variant}\index{termination}
...
returns the base address of the block containing an allocated pointer
\begin{listing-nonumber}
\base_addr : $\alpha$ * $\ra$ char*
\end{listing-nonumber}
\item \blocklength\indexttbs{block\_length}
  returns the length of the allocated block of a pointer in bytes
\begin{listing-nonumber}
...
\lstinline|\valid(s)| holds if and only if
 dereferencing any $p\in \lstinline|s|$ is safe, both for reading from 
...
\lstinline|\valid(p)| implies \lstinline|\valid_read(p)| but the reverse is 
not true. In particular, it is allowed to read from a string literal, but not 
to write in it (see \cite{standardc99}, section 6.4.5.6).

Note in addition that the status of \valid depends on the
type of its argument. Namely, \lstinline|\valid((int *) p)| and 
\lstinline|\valid((char *)p)| are not equivalent. On the other hand, if we 
ignore potential alignment constraints, the following equivalence is true:
\begin{listing-nonumber}
