...
variable refered to outside of its scope, or the address of a variable that
has been dynamically allocated, then deallocated.
\\ \makebox[5mm]{} \lstinline|\dangling_contents{id} : set<$\alpha$*> $\ra$ bool|
...
In most cases, the arguments to \lstinline|\dangling_contents| are pointers to
l-values that themselves have type pointer, so the usual signature
of \lstinline|\dangling_contents| is actually
