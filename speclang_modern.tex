...
\subsubsection{Size and alignment of C types}
\label{sec:sizeof-alignof}

The size and alignment of C types is architecture-dependent. ACSL does
not enforce these either, hence the semantics of terms involving
such types is also architecture-dependent. The
\lstinline|sizeof|\indextt{sizeof} and \lstinline|alignof|\indextt{alignof}
operators may be used in annotations and are consistent with their C counterpart
(including that their return type is a value of type \lstinline|size_t|, and in
most cases a constant). For instance, it should be possible to verify the
following code:
...
\label{sec:pre-post-state}
A function call involves two particular program states: the
\emph{pre-state}, when entering the function, and the \emph{post-state},
when returning back to the caller (see Sect.~\ref{sec:termination}
for functions that may not terminate and Sect.~\ref{sec:abrupt-clause}
for functions that may exit the program).
...
\end{itemize}
\lstinline|\result| and \lstinline|\old(e)| can be used only in \ensures{}, \assigns{},
\allocates{} and \frees{} clauses, since
the other clauses already refer to only one state, the pre-state.

Moreover, the formal parameters of the function are \emph{not} allocated in the post-state.
Hence, in the post-state, any formal parameter \lstinline|x| can
only be evaluated if it is enclosed in a term of the form \lstinline|\old(e)|.
In order to keep formulas simpler, though, \lstinline|x| can be used instead of
\lstinline|\old(x)|.
...
\lstinline|ensures| clauses default to \lstinline|\true|. Missing
\lstinline|exits| clauses default to \lstinline|\false|.
...
\ifCPP{\review{Namespaces provide also this capability.}}

Specification modules can be provided to encapsulate
...
\lstinline|List::length|.

Modules can be nested and it is also possible to use qualified names
for defining modules. Consider for instance:
\begin{lstlisting}
/*@
  module A {
    module B::C { ... }
 ...
 */
\end{lstlisting}
This will define three modules named \verb+A+, \verb+A::B::C+ and
\verb+A::B::D+.

It is not possible to define two modules with the same fully qualified name.
When using long qualified names, it is possible to use import directives. An
import directive has the following format:

\begin{lstlisting}
//@ import <qualified-name> [ \as <identifier> ] ;
\end{lstlisting}

When importing a module, its qualified name can be shortened to its last
element. For instance, when importing module \verb+A::B::C+, it can be shortened
as \verb+C+. When the import clause uses the \verb+\as X+ form, the full module
name can be shortened with \verb+X+ instead. Notice that imported symbols can
still be accessed using their fully qualified names. Only complete modules or
sub-modules can be imported, not individual logic declarations.

It is possible to import a given module several times, under any name. Priority
is given to the last module imported. To avoid misleading references, when
importing several modules under the \emph{same} name \verb+X+, only the last
module becomes visible: all symbols from previously imported modules under the
same short name are hidden by the newly imported module, even for symbols that
are not re-declared.

Import clauses can be used inside module definitions, with a scope limited to
subsequent declarations and sub-modules of the current module.
...
Moreover, some operations involving pointers may lead to runtime errors.
According to the C standard~\cite{standardc99}, several properties must hold
when reading, creating or dereferencing a pointer. The following built-in
predicates deal with these notions.


\begin{itemize}
\item \lstinline|\valid_read{L}(s)|\indexttbs{valid\_read} applies to a tset
(see Section~\ref{sec:locations}) of some pointer to an object type and holds if
and only if at label \lstinline|L|, for any $p\in \lstinline|s|$
\begin{listing-nonumber}
\base_addr(p) <= p && p + sizeof(*p) <= \block_length(p)
\end{listing-nonumber}
In particular, \lstinline|\valid_read{L}(\empty)| holds for any label \lstinline|L|.
\\ \makebox[5mm]{} \lstinline|\valid_read{id} : set<$\alpha$ *> $\ra$ boolean|
\item \lstinline|\valid{L}(s)|\indexttbs{valid} is \lstinline|\valid_read{S}(l)|,
plus all memory locations are writable.
\\ \makebox[5mm]{} \lstinline|\valid{id} : set<$\alpha$ *> $\ra$ boolean|
\end{itemize}

\lstinline|\valid{L}(s)| implies \lstinline|\valid_read{L}(s)| but the reverse
is not true. In particular, it is allowed to read from a string literal, but not
...
ignore potential alignment constraints, the following equivalence is true for
any pointer \lstinline|p|:
\begin{listing-nonumber}
...
In addition to this notion of validity, it should be noted that
adding or subtracting an offset to a pointer, casting a pointer to another type
(or building it from an integer), or comparing two pointers may also lead to
undefined behaviors (see \cite[6.3.2, 6.5.6 and 6.5.8]{standardc99}).
Predicates \lstinline|\object_pointer| and \lstinline|\pointer_comparable|
deal with these notions.
\begin{itemize}
\item \lstinline|\aligned|\indexttbs{aligned} applies to a pointer and a value
which is the expected alignment. \lstinline{\aligned(p, n)} indicates that
\lstinline|p| is aligned modulo \lstinline{n}.
\item \lstinline|\object_pointer|\indexttbs{object\_pointer} applies to a tset of
...
\item \lstinline|\pointer_comparable|\indexttbs{pointer\_comparable} takes two pointers
...
\paragraph{Remark:} The validity (according to \lstinline|\valid|) of the pointer
does not interfere with initialization in the sense of this predicate, these are
two separated notions that can be treated in isolation from each other. For
example, \lstinline|\initialized(p) ==> \valid_read(p)| is not an axiom.
