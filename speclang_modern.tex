...
/*@ \allocates $P_1$, $P_2$, ...
  @ \frees  $Q_1$, $Q_2$, ... 
\end{listing-nonumber}
means that all memory addresses that do not belong to the set 
$P_1 \union P_2 \union ...  \union \old(Q_1 \union Q_2 \union ...)$ 
(set evaluated in the post-state) refer to memory those allocation statuses
...
\item \begin{notimplementedenv}\lstinline|\free{L}(p)|\end{notimplementedenv}%
\indexttbs{free},
