...
argument and means that each l-value in this set has a \emph{specified
  value} at label \lstinline|L|: its value is not a dangling pointer.
(``Not dangling'' means that the value is neither the address of a local
variable refered to outside of its scope, nor the address of a variable that
...
In most cases, the arguments to \lstinline|\specified| are pointers to
l-values that themselves have type pointer, so the usual signature
of \lstinline|\specified| is actually \lstinline|set<$\alpha$**> $\ra$ bool|.
