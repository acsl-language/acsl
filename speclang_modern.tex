...
\item Comments may be put inside ACSL annotations. They use the C++
  ...
  \item Some grammar elements are incorporated from C \ifCPP{or CPP}.
  \begin{itemize}
  \item An \textit{integer literal} is a digit sequence with optional radix and bit-size indicators. TBD
  \item A \textit{real literal} has the form TBD
  ...
  preceded by the \lstinline|L| character to denote a wide character: TBD
...
\begin{itemize}
\item An \lstinline|id| is a basic alphanumeric identifier, used to denote all manner of language constructs.
\item A \lstinline|poly-id| is an \lstinline|id|, possibly with formal type or label arguments 
...
\lstinline|$t_1$ $relop_1$ $t_2$ $relop_2$ $t_3$ $\cdots$ $t_k$| with
  ...
  Furthermore the tdypes of each of the terms being compared must be non-boolean.
...
\ifCPP{
\paragraph{\lang{}} typing of \lstinline|bool| and \lstinline|boolean|}
...
As there are no difference in range, \lstinline|bool| and 
\lstinline|boolean| may be implicitly converted to each other.

Unlike C, \lang{} does distinguish \lstinline|bool| and integral types, but 
...
\begin{itemize}
	\item Under \lstinline|writes| (or assigns) semantics, only those memory locations listed in a frame condition may be \emph{written to}, that is, only those locations may be the target of an assignment statement. This is true whether or not the value of the memory location changes.
...
/*@ `\dots`
  ...
  @*/
\end{listing-nonumber}
means that all named behaviors given in the contract should be taken into account.\footnote{If there is a default (unnamed) behavior, it has an 
	\lstinline|assumes| clause of true; including it makes the 
	completeness assertion trivially true.}
...
\end{listing-nonumber}
means that all named behaviors given in the contract should be taken into account.\footnote{If there is a default (unnamed) behavior, it has an 
\lstinline|assumes| clause of true and is thus not disjoint with other clauses.}
...
The constructs in Figure~\ref{fig:gram:locations} are syntactically valid whatever the types of terms are, but are only type-valid when used on values that are tsets as described.
\footnote{Resolving the restrictions on tsets during type-checking rather than parsing greatly reduces the size and complexity of the overall grammar and avoids needing to
...
is the following (in \lang{}, this specification would also include \lstinline|throws \false;|):
\begin{listing}{1}
/*@ ensures \false;
    terminates \false;
*/
...
The axiomatic construct is solely a grouping construct. Currently the grammar 
...
\ifCPP{\input{cpp-class-invariants}}
