...
\ifCPP{\TODO{Check this list. Should min max be n-ary}}
...
\ifCPP{\TODO{Exact computations needs clearer explanation}}
...
\ifCPP{\TODO{SHould there be a grammar for the above?}}
...
\ifCPP{\TODO{Explain the above about equality of unions}}
...
\lstinline|assigns \union(x,{y});|. If neiether \lstinline|x| and \lstinline|y| are sets, then \lstinline|assigns x,y;| means
...
\TODO{This rule seems not to be sound. Better would be a single-definition rule -- the same contract must be visible for all uses and implementations of a function. Repetitions of a contract must be token for token identical. }
...
\begin{example}
\label{redeclaration}
\inputlisting{redeclaredat.c}
\end[example}
\begin{itemize}
\item The assert annotation on line 9 refers to the value of \lstinline|x| declared on line 1 and set on line 6; it is proved without difficulty.
\item The assert annotation on line 15 refers to the current state of \lstinline|*x|, where \lstinline|x| is declared on line 10, that is the assignment to \lstinline|y| on line 13; it is also proved without difficulty.
\item The assert annotation on line 16 refers to the state of \lstinline|*x| at label \lstinline|c|, that is the assignment to \lstinline|y| on line 5; it is also proved without difficulty.
\end{itemize}
But consider the assert annotation on line 14. It references the state at label b. At that label, \lstinline|x| refers to the declaration on line 1, not the declaration current at line 14, namely the declaration on line 10. 
...
types\index{type!sum}, product (tuple) types, and function types. These definitions may be recursive.
...
\item \lstinline|\null|\indexttbs{null} is an
  ...
  null pointer (\emph{i.e.} a shortcut for \lstinline|(void*)0|)\ifCPP{ and, in \lang, for \lstinline|nnullptr|}. 
...
some pointer type other than \lstinline|void*|
...
\TODO{Should set be \textbackslash set}
...
\begin{itemize}
	...
	\item let statements with multiple parallel local bindings
