...
Nevertheless, arithmetic and dereferencing over arrays lying in C memory are allowed like in C.

\begin{example}
~
Dereferencing a C array is equivalent an access to the first element of the array~;
...
Since pointers can only refers values lying in C memory, 
\lstinline|p->s| is always equivalent to \lstinline|(*p).s|.
On the contrary, \lstinline|t[i]| is not always equivalent to 
\lstinline|*(t+i)|, especially for arrays lying not in C 
(see Section~\ref{sec:aggtregate} for details about arrays 
as possible logic values).
...
in Section~\ref{sec:locations}. \textsl{abrupt-clauses}
...
applies only for behaviors \lstinline|id_1|,$\ldots$,\lstinline|id_k|, hence in
...
post-state when the annotated statement terminates abruptly
...
Section~\ref{sec:abrupt-clauses}) need to be used.
...
% PB: Modified since \separated operates on pointers and not on locations
\lstinline|\separated($pt_1$,..,$pt_n$)|\indexttbs{separated}
means that for each
$i\neq j$, the intersection of locations \lstinline|*|$pt_i$ and \lstinline|*|$pt_j$ is empty. Each
$pt_i$ is a set of terms of some pointer type as defined in Section~\ref{sec:locations}.
...
does not constraint the post-state when the annotated function and
...
\begin{example}
  ...
  function is incomplete allows modifications of the variable
