...
/*@ frees P$_1$,P$_2$,`\dots`;
  ...
  @*/
\end{listing-nonumber}
means that any memory address that do not belong to the union of sets of terms
...
\Loop \allocates and \Loop \frees have the same semantics.
\begin{listing-nonumber}
//@ loop allocates Q$_1$,Q$_2$,`\dots`;
\end{listing-nonumber}
means that any memory address that do not belong to the union of sets of 
terms
...
before entering the loop. Namely, as for \Loop \assigns, 
\Loop \allocates defines a loop invariant.

More precisely, with \lstinline|L$_0$| labelling the state before entering
the loop, the clause \Loop \allocates \lstinline|Q$_1$,$\ldots$,Q$_n$|
...
It is the reason why the \lstinline|loop frees| clause requires 
the use of the ghost label \lstinline|L0|,
