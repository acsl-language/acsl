...
%% \begin{remark}{Claude} et les long double
%% \end{remark}

%% \oldremark{Claude}{on pourrait enonce un certain nombre de proprietes:
...
%%   Il faut indexer ces fonctions et mentionner quelque part que la surcharge 
%%   des finctions logiques est possible.
%% \end{remark}
...
%% \oldremark{Colaco}{les assigns doivent etre exhaustifs, 
%% et doivent etre prouve corrects selon le corps de la function.}
...
%%
%%  La condition de d'exclusion mutuelle des cas de behaviors est
%%    pour toute paire de behavior $B_1$ et $B_2$ la postcondition
%%    supplementaire est :
%%     not (     (\old(assumes_1) and postassume_1)
%%           and (\old(assumes_2) and postassume_2))
%%
%%  La condition de couverture des cas de behaviors est :
%%    la disjunction des $(\old(assumes_i) and postassume_i)$ est une 
%%    postcondition supplementaire}
...
%% \oldremark{Claude}{TODO(citer une ref a propos des invariants inductifs) }
...
%%  sucre syntaxique: t[i] pour select(t,i),  
%%  et dans le cas de code ghost, t[i] = e pour t = store(t,i,e)
%%
%%  Example: n-reines.
%% }
...
%%   \oldremark{Claude}{expliquer mieux
%%
...
\section{Sets and lists as first-class values}
...
\end{listing-nonumber}
while \lstinline|\concat|\indexttbs{concat} concanates two sequences
\begin{listing-nonumber}
\list<A> \concat<A>(\list<A> front, \list<A> tail);
\end{listing-nonumber}
and \lstinline|\repeat|\indexttbs{repeat} repeats a sequence a positive number
\lstinline|n| of times
\begin{listing-nonumber}
...
Namely, the first element is at position $0$. 
\begin{listing-nonumber}
<A> \nth<A>(\list<A> seq, integer n);
\end{listing-nonumber}

Last but not least, the functions \lstinline|\nth| and \lstinline|\repeat| 
aren't defined for negative number \lstinline|n|.
...
%% \oldremark{Claude}{et les var locales d'une fonction appelante ?}
...
%% \oldremark{PC}{remise en cause de la politique d'invariants:
...
%% \oldremark{Claude}{d'autres exemples, valides ou non valides, 
%%   serait bienvenus 
%% }

%% \oldremark{All}{ On veut des tableaux ghost de taille infinie 
%%  -> cf tableaux fonctionnels de la section sur les assigns from
%% }
