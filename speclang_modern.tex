...
consistent with its C counterpart (including that its return type is an integer constant). For instance, it should be
...
\marginpar{Why is there an anomalous space before the requires at the beginning of the paragraph?}
...
\item At any loop iteration, any location that was allocated before
  ...
  \marginpar{by 'current state' do we mean Here or LoopCurrent or something else?}
...
\marginpar{Is \textit{and any \at construct can modify the interpretation of formal parameters} correct? I thought formal parameters were always in the Pre state despite any at construct.}
...
\marginpar{May behavior ids of statement contracts shadow the ids of the enclosing function constract? Or are identical names forbidden? The latter is the easier case.}
\marginpar{Should forbid empty behavior-body-stmt}
...
\end{itemize}
\marginpar{Define base\_addr and block\_length on null ?}
...
available.
\marginpar{Why are Nil and Cons capitalized but not the others?}
...
\item A weak type invariant on type $\tau$ must hold at any function
  ...
  \marginpar{Why does the invariant not apply to everything of the given type?}
...
\end{example}
\marginpar{Why is the invariant not strong in the Dutch flag example? Also, perhaps such an invariant should be automatically generated for an enum. }
...
\item It is however possible to write multi-line annotations inside ghost
  ...
  considered as blank. \marginpar{Is this form of multi-line comments embedded in multi-line C (or ACSL) comments generally applicable?  If so, it should be introduced in section 1.2.}
...
\marginpar{It seems silly to have a composite section title and then separate subsections for each of the pieces. Perhaps just make the two subsections sections themselves.}
