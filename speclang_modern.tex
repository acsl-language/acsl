...
\item Some UTF8 characters may be used in place of some constructs, as
  ...
  \begin{center}
    \begin{tabular}{|l|l|l|}
      ...
      \hline
      \lstinline+^^+ & \ensuremath{\oplus} & 0x22BB \\
      \hline
      \lstinline+!+ & \ensuremath{\neg} & 0x00AC \\
      \hline
...
\item[Additional connectives] C operators \lstinline|&&| (UTF8:
  ...
  equivalence and \lstinline|^^| (UTF8: $\oplus$) for exclusive
...
\item[Local binding]
    ...
    expression \lstinline|$e_1$| which can be used in expression
    \lstinline|$e_2$|.
...
\item[Functional modifier]
 \begin{notimplementedenv}
  ...
  denotes the same structure than \lstinline|s|, except for the field
  \lstinline|id| to be modified by \lstinline|v|.
  The equivalent expression for an array is
  \lstinline|{ t \with [ i ] = v }|
  which returns the same array than \lstinline|t|, except for the
  \lstinline|i$^{th}$| element those value
  is updated to \lstinline|v|.
...
is equivalent to
\begin{listing-nonumber}
\forall integer c; MIN_CHAR <= c <= MAX_CHAR ==> c <= 1000
\end{listing-nonumber}
where the the bounds \verb|MIN_CHAR| and \verb|MAX_CHAR| are defined
in \verb|limits.h|
\end{example}
...
Note that the size of C types is architecture-dependent. ACSL does
...
mathematical integer: any mathematical integer has an unique infinite
...
real numbers, hence they never involve any rounding nor overflow
...
$+\infty$ and $-\infty$. See below for a detalied discussions on such
...
Finally, additional predicates are provided on float and double
numbers, which check that their argument is NaN or a
finite number respectively:
\begin{listing}{1}
predicate \is_NaN`\indexttbs{is\_NaN}`(float x); \\
predicate \is_NaN`\indexttbs{is\_NaN}`(double x); \\
predicate \is_finite`\indexttbs{is\_finite}`(float x); \\
predicate \is_finite`\indexttbs{is\_finite}`(double x);
\end{listing}
...
\begin{remark}{Virgile}
  Il faut indexer ces fonctions

  Il faut mentionner quelque part que la surcharge des finctions
  logiques est possible.
...
\begin{remark}{???}
The type system should provide a boolean information on whether a
given logic term denotes a C memory location (i.e. a C left-value).
\end{remark}

Unlike C, there is no implicit cast from from an array type to a pointer type.
Nevertheless, arithmetic and dereferencing over arrays lying in C memory are 
allowed like in C.

\begin{example}
~
Dereferencing a C array is equivalent an access 
to the first element of the array~;
shifting it from $i$ denotes the address of it $i^{th}$ element.
\begin{listing-nonumber}
int tab[10] = { 1 } ;
int x ;
int *p = &x;
...
Since pointers can only refers to values lying in C memory, 
...
t[i] can be written even if i is outside the array bounds.
...
There is no particular syntax for a functional update of an union, but of course the following
equility is not be true for an \lstinline|object| of an union type:
\begin{listing-nonumber}
{ { object \with .x = 3.0 }
...
\begin{remark}{}
other built-in constructs: array.sub ? array.map ? (later)
\end{remark}



The equality predicate \lstinline|==| applies to aggregate values, but it
is required that they have the same type. Then equality amounts the
recursively check equality of fields.  Equality of arrays of different
...
C aggregates types (struct, union or array) naturally map to logic
...
for arrays that lies in C memory. As in C, the result of the cast is
the address of the first element of the array (see
Section~\ref{sec:address}).

Conversely, an explicit cast from a pointer type to an array type or a
structure type is allowed, and acts as collecting the values it points
to.
...
(p) - 
\begin{notimplementedenv}\lstinline|\offset|\end{notimplementedenv}%
(p))/\lstinline|sizeof|(\tau)
\]

\subsection{String literals}

\begin{remark}{Pascal, Benjamin}

Quelle est la semantique des constanes chaines dans les annotations
logiques ?

Une reponse simple possible : equivalent a une constante globale ghost


"foo" devient:


ghost char string\_literal\_foo [4] = "foo" (avec le 0 final)

et utiliser string\_literal\_foo a la place de "foo"
...
Note C function parameters\index{formal parameter} are obtained by
...
So, \old construct is useless for formal parameters (in
...
\item All memory locations of the pre-state that do not belong to the
  set \lstinline|L$_1$ $\cup$ L$_2$ $\cup \dots$| remain allocated and are left unchanged in the
  post-state. The set \lstinline|L$_1$ $\cup$ L$_2$ $\cup \dots$| itself is interpreted in the
  pre-state.
\end{itemize}
...
\item For each $i$, if the function is called in a pre-state where
  ...
  that does not belong to the set \lstinline|L$_i$| remains allocated 
  and is left unchanged in the post-state.
...
/*@ requires P && (A$_1$ ==> R$_1$) && (A$_2$ ==> R$_2$);
  @ assigns L$_1$,L$_2$;
  @ ensures \old(A$_1$) ==> E$_1$ && \old(A$_2$) ==> E$_2$;
  @*/
\end{listing}
...
\label{sec:locations}\indextt{location}

% PB: Modified since \separated operates on pointers and not on locations
...
More generally, we introduce syntactic constructs to denote \emph{sets of
  terms} which are also usefull for \separated predicate (see Section~\ref{sec:separated})
...
\item At any loop iteration, any location that was allocated before
  ...
  invariant for the locations that are not member of \lstinline|L|.
\end{itemize}

\subsubsection{Loop behaviors}
\index{loop!behavior}
A loop annotation preceded by
\lstinline|for id_1,$\ldots$,id_k:| is similar as above, but
...
\paragraph{Remarks}
\begin{itemize}
\item \begin{notimplementedenv}[This is not checked]
    The \old{} construct is not
    allowed
    \end{notimplementedenv}
...
Note that the value of \lstinline|m| at loop exit might be
...
\lstinline|s|. Such an invariant is a kind of assertions, as
...
There are four predefined logic labels: \lstinline|Pre|, \lstinline|Here|,
\lstinline|Old| and
\lstinline|Post|. \lstinline|\old(e)|\indexttbs{old} is
...
at the begining of the annoted statement and leading 
...
\begin{figure}[t]
  \fbox{\begin{minipage}{0.97\linewidth} \input{inductive_modern.bnf}
    \end{minipage}}
...
\begin{example}
  The following introduce a predicate \lstinline|isgcd(x,y,d)| meaning
...
\index{axiomatic}
\begin{figure}[t]
  \fbox{\begin{minipage}{0.97\linewidth} \input{logicdecl_modern.bnf}
    \end{minipage}}
...
for axiomatic declaration of \lstinline+f+ with an axiom
...
\begin{example}
  ...
  condition of a sorting function would be \lstinline|permut{Pre,Post}(t,t)|.
...
Logic declaration can be augmented with a \reads{} clause, with the
...
memory locations which it depends on. From such an information, one
...
\begin{remark}{Claude}
on peut indiquer des conditions semantiques de validite des axiomes:

pour tout etat L1, L2 qui sont identiques sur le footprint, P{L1} =>
P{L2} ou P est la formule de l'axiome
...
%   operator is mandatory here. Indeed the two arrays
%   may come from two distinct memory states. Typically, one of the post
%   condition of a sorting function would be \lstinline+permut{Pre,Here}(t,t)+.
%   \listinginput{1}{permut_reads.c}
% \end{example}
...
\listinginput{1}{import.c} where the file \lstinline|list.acsl| contains logic
...
\label{sec:pointers} 
% [Patrick B.] START proposition:
First, the grammar for terms and predicates is
...
function (when the allocation succeed) and their length.

ACSL provides the following built-in functions to deal with allocated blocks.
All of them takes an optional label identifier as argument. 
...
\item 
\begin{notimplementedenv}
[Actual implementation recognizes only one type of validity]
\lstinline|\valid_read|\indexttbs{valid_read}
\end{notimplementedenv}
...
Note in addition that the status of \valid and \lstinline|\valid_read| constructs depends on the
...
\begin{listing-nonumber}
\valid{L}(p) <==> \valid{L}(((char *)p)+(0..sizeof(*p)))
\end{listing-nonumber}
and
\begin{listing-nonumber}
\valid_read{L}(p) <==> \valid_read{L}(((char *)p)+(0..sizeof(*p)))
\end{listing-nonumber}
...
\item \lstinline|\null|\indexttbs{null} is an
  ...
  null pointer (\emph{i.e.} a shortcut for \lstinline|(void*)0|). Note that
  as in C itself (see \cite{standardc99}, 6.3.2.3\S3),
  the constant \lstinline|0| can have any pointer type. In addition, 
  \lstinline|\valid{L}(\null)| is always false, for any logic label \lstinline|L|.
...
\item \begin{notimplementedenv}\lstinline|\offset{L}(p)|\end{notimplementedenv}%
  \indexttbs{offset}
...
\\ \makebox[5mm]{} \begin{tabular}{lll}
  ...
  \end{tabular} \\
Again, if there is no alignment constraints,
...
\subsection{Dynamic allocation and deallocation}\label{sec:allocation-clause}
...
The semantics of the clauses \allocates and \frees cannot be defined separately from the other.
The simple contract  
\begin{listing-nonumber}
/*@ \frees P$_1$,P$_2$,`\dots`;
  ...
  @*/
\end{listing-nonumber}
means that all memory addresses, that do not belong to the union of sets of terms
\lstinline|P$_i$| and \lstinline|Q$_j$| of some pointer type,
refer to memory those {\sl allocation statuses}
in the post-state are identical to those in the pre-state. 
While the sets \lstinline|P$_i$| are evaluated in the pre-state,
the sets \lstinline|Q$_i$| are evaluated in the post-state.
That is the only one difference between the clauses \allocates and \lstinline|frees|.

Lets now define what is meaning by unmodified {\sl allocation statuses}:
the clause \allocates \lstinline|Q$_1$,$\ldots$,Q$_n$| is equivalent
to the postcondition
\begin{listing-nonumber}
\forall char* p; 
\separated(\union(Q$_1$,$\ldots$,Q$_n$),p)==>(\allocation{Here}(p)==\allocation{Pre}(p)
...
In fact, like the \assigns clause does not specify which memory location is assigned,
...
about these allocations and deallocations.
The following built-in constructs can be used for that:

\begin{itemize}
\item  \begin{notimplementedenv}\lstinline|\allocation{L}(p)|\end{notimplementedenv}%
  \indexttbs{allocation}
 informs on the allocation status of the block containing, at the label \lstinline|L|, the pointer \lstinline|p| 
\\ \makebox[5mm]{} \lstinline|\allocation{id} : void* $\ra$| 
\begin{notimplementedenv}\lstinline|{\static, \register, \automatic, \dynamic, \unallocated}|
\end{notimplementedenv}%
...
and
\begin{listing-nonumber}
\allocation{L}(p)==\unallocated ==> !\valid_read{L}(p)
\end{listing-nonumber}
\end{itemize}

Some usefull shortcuts are provided:

\begin{itemize}
\item \begin{notimplementedenv}\lstinline|\allocable{L}(p)|\end{notimplementedenv}%
  \indexttbs{allocable}
 holds if and only if
the pointer \lstinline|p| refers, at the label \lstinline|L|, the base address of 
...
\item \begin{notimplementedenv}\lstinline|\freeable{L}(p)|\end{notimplementedenv}%
  \indexttbs{freeable}
 holds if and only if
the pointer \lstinline|p| refers, at the label \lstinline|L|, an allocated memory block 
...
\item \begin{notimplementedenv}\lstinline|\fresh{L$_0$,L$_1$}(p,n)|\end{notimplementedenv}%
\indexttbs{fresh},
  indicates that \lstinline|p| refers an allocated memory block at label \lstinline|L$_1$|, 
...
\fresh{L$_0$,L$_1$}(p,n) <==> (\allocable{L$_0$}(p) && 
                         ...
                         \valid{L$_1$}((char*)p+(0..n))
\end{listing-nonumber}
\end{itemize}

\begin{example}
  There is a first example giving specifications to \lstinline|malloc| and \lstinline|free| functions.
  \listinginput{1}{malloc_free_fn.c}
  Default labels for constructs dedicated to memory are 
  such that logic labels \lstinline|Here| and \lstinline|Old| can be omitted.
\end{example}

When a behavior has only one of the two allocation clauses,
...
Now, when none of the two allocation clauses is given, the meaning is different
...
\item for an anonymous behavior, no allocation clause means that there is no newly 
      ...
      That is equivalent to give \allocates \nothing.
\end{itemize}
These rules are such that contracts without any allocation clause should be considered 
as having only one \allocates \nothing leading to a condition to verify for 
...
\begin{example}
  ...
  The behaviors named \lstinline|allocation| and \lstinline|deallocation| do not 
  ...
  To set a stronger contraint into the behavior named \lstinline|no_allocation|,
...
Loop annotations are complemented by similar clauses allowing to specify which memory location 
is dynamically allocated or deallocated by a loop.
The grammar for those constructions is given on Figure~\ref{fig:gram:allocation}.

The semantics of the \Loop \allocates clause is defined as follows:
\begin{listing-nonumber}
//@ loop allocates Q$_1$,Q$_2$,`\dots`;
\end{listing-nonumber}
means that all memory addresses, that do not belong to the union of sets of terms
\lstinline|Q$_1$ $\cup$ Q$_2$ $\cup \dots$| (interpreted in the current state), 
refer to memory those {\sl allocation statuses}
have the same value in the current state than before entering the loop.

Lets name \lstinline|Here| the current state and \lstinline|L$_0$| the one before entering the loop
for defining what is meaning by unmodified {\sl allocation statuses}:
the clause \Loop \allocates \lstinline|Q$_1$,$\ldots$,Q$_n$| is equivalent
to the postcondition
\begin{listing-nonumber}
\forall char* p; 
\separated(\union(Q$_1$,$\ldots$,Q$_n$),p)==>(\allocation{Here}(p)==\allocation{L$_0$}(p)
                               && \base_addr{Here}(p)==\base_addr{L$_0$}(p)
                               && \block_length{Here}(p)==\block_length{L$_0$}(p)
                               && \valid{Here}(p)<==>\valid{L$_0$}(p)
                               && \valid_read{Here}(p)<==>\valid_read{L$_0$}(p))
\end{listing-nonumber}


\begin{example}
The addresses of locations \lstinline|q[0..n]| are not modified by the loop, but their values are.
  \listinginput{1}{loop-frees.c}
It is the reason why the \Loop \allocates clause requires the use of the ghost label \lstinline|L0|,
in order to catch the set of the released memory blocks.
\end{example}

\section{Sets as first-class values}\label{sec:sets}
\index{location}
...
\begin{example}
  ...
  Notice that in the first definition, since union is made with a
  \lstinline|set<char*>| and a \lstinline|set<int*>|,
  the result is a \lstinline|set<char*>| (accordingly to typing of union). In
...
logics labels\indextt{Pre}\indextt{Here}\indextt{Old}\indextt{Post}
...
construct is allowed and bounds to the returned value (if not a
\void{} function).
...
The \exits{} clause can be used either in function or statement
...
The  construct \lstinline|\exit_status|\indexttbs{exit_status} can be used only in \exits{},
...
\begin{itemize}
\item functional  of array : \notimplemented{$\{ t \lstinline$\with$ [i] {\tt=} e \} $}
\item functional update of struct : \notimplemented{$\{ s \lstinline$\with$ f {\tt=} e \} $}
...
\item A weak type invariant on type $\tau$ must hold at any function
  ...
  weak invariant at function exit. However, it says nothing of fields,
...
\item It is however possible to write multi-line annotations for ghost
  ...
  As in normal annotations, \lstinline|@|s at the beginning of a
  line and at the end of the comment (before the final \lstinline|@/|) are
  considered as blank.
  \oldremark{Claude}{
...
\lstinline|\initialized| is a predicate taking a set of l-values as
...
\indexttbs{specified}
\begin{notimplementedenv}\lstinline|\specified|\end{notimplementedenv}
is a predicate taking a set of l-values as
argument and means that each l-value in this set has a \emph{specified
  value}: its value is not a dangling pointer (that is, the value
is not the address of a local variable refered to outside of its scope)

\begin{example}
  In the following, the assertion is not true.
  \listinginput{1}{specified.c}
\end{example}
