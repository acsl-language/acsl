\section{Access control and abstraction}
\label{sec:access}
...
\lstinline|private| in \lang{}, since that is the most recent \lang{} access specifier, but until the next access specifier, the declarations have \lstinline|public| access within specifications. \TODO{Perhaps this should apply to just the immediate declaration?}
...
The access specifier \lstinline|spec_protected| may also be seud (within \NAME{} annotations) to give \lang{} \lstinline|protected| access to \lang{} constructs
...
\TODO{Should we insist on strong purity -- no changes to the heap at all? -- or weak purity -- no changes to the prestate and no allocations that are not deallocated by the post-state?}

\subsubsection{... other  conditions...}

\TODO{What other conditions, e.g. reading from the heap?}


\TODO{Also need to state rules about state parameters for logic functions when it have a 'this' parameter}
