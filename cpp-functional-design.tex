...
specification interfaces (\S\ref{TBD}) for lambda expressions; 
reasoning about functional objects is accomplished using
dynamic contracts (\S\ref{reasoning}). 
These mechanisms are then demonstrated with examples and use cases in \S\ref{examples}ff.
...
`naked' function pointers and functor objects is discussed in
Section \ref{TBD}; 
specifying and reasoning about functor objects is presented in Section \ref{TBD}.
...
pointer is optional for static functions. The conversion is performed implicitly where needed. The \lstinline|&| is needed to take the address
...
TODO: What about captures clauses
...
So the use of \lstinline|&| is optional.
...
different textual functions that implement precisely the
...
Conventional function definitions have an established place to place their specification (cf. \S\ref{TBD}). Functions defined by lambda expressions do not. These appear in the middle of expressions where the insertion of a function contract would be intrusive and difficult to read.
...
function's arguments and possible the program state that is 
...
\lstinline|\requires(fp,\emph{args})|. 
...
\begin{itemize}
	...
	given by the single clause combined across all the
	...
	\lstinline|\result|, 
	\lstinline|\exception|, |\exit\_status|). 
...
\end{listing-nonumber}
	...
	result value and argument values for which the ensures clause would be true. This incorporates any underspecification of the function.

\end{itemize}

\subsection{Member function pointers}
Member function pointers occupy a different region of type space than do static functino pointers, but are nevertheless treated quite
...
\begin{lstlisting}
  \forall A this; \requires(|A::m|,this) == this->x > 0;
  \forall A this; \assigns(|A::m|,this) == \empty;
  \forall A this, int result; \ensures(|A::m|,this,result) == (result == this->x + 1);
\end{lstlisting}
...
\begin{itemize}[noitemsep,nolistsep]
	...
	\item assume the preconditions of the function
	\item translate the body of the function
	\item assert (to be proved) the postconditions of the function
\end{itemize}
...
\begin{itemize}[noitemsep,nolistsep]
	...
	function pointer literal for the \lstinline|increment| function
	...
	\lstinline|fp|; line 6 is the translation of 7 of the C++ code above.
	...
	\lstinline|\empty|).
	\item Line 10 gives a state label to the post-state of the function call
...
Here since the functions and function pointers have a void return type, there is no argument of the \lstinline|\ensure| term corresponding to the return value. Also, the \lstinline|test| function has no frame condition, 
...
\end{itemize}
The frame conditions here are \emph{dynamic}: they are not simply a list of statically known memory locations. The logical encoding and
...
This subsection begins a series of examples of such methods might be 
...
\lstinline|operator()|. Functors can be sued in functional programming environments, in which case it is the special 
\lstinline|operator()| that is called. For example, as instance of a functor class can be passed to 
...
put a function contract for the lambda expression. We use the same solution as above, namely specification classes and casts.
...
TODO: what about quantification over labels

TODO: Check all uses of labels in the examples
