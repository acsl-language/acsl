...
\item ACSL is a BISL for C, a low-level structured language, while JML
  ...
  properties on memory (e.g., the size of an allocated block).
\item JML relies on runtime assertion checking (RAC) when typing,
  ...
  in a modular way. Indeed, it is necessary to modify the layout of
  data structures for RAC, which is not modular. The follow-up project
  Deputy~\cite{condit07deputy} thus reduces the checking power of
  annotations in order to preserve modularity.  On the contrary, we
  choose not to restrein the power of annotations (e.g., all first
  order logic formulas are allowed). To that end, we rely on manual
  deductive verification using an interactive theorem prover (e.g.,
  Coq) when every other thing failed.
\end{itemize}

\noindent
In the following, we further describe a few important consequences of
these differences.
...
\begin{example}
In \S 7.19.5.1 of the standard, it is said that function fclose signal
errors as follows:
...
\noindent
A tool could easily set errors conditions aside, by providing an
...
on the length of array {\ttfamily a}, it could easily expressed using
the JAVA expression {\ttfamily a.length}.

\begin{flushleft}\ttfamily
public static void nullify(char[] a) \{ \\
~~if (a == null) return; \\
...
or rather a lower bound of this size. 

\begin{flushleft}\ttfamily
void nullify(char* a, unsigned int n) \{ \\
~~int i; \\
~~if (n == 0) return; \\
~~for (i = 0; i < n; ++i) \{ \\
~~~~a[i] = 0; \\
...
with the meaning that $\valid(a + 0..(-1))$ is true.
When {\ttfamily n} is null, {\ttfamily a} does
not need to be valid at all, and when {\ttfamily n} is stictly
positive, {\ttfamily a} must point to an array of size at least
{\ttfamily n}.  To make it more obvious, the C programmer added a
defensive programming style, which returns when {\ttfamily n} is
...
behaviors, selected according to some tests. Since C has no memory
...
This is why it is possible in ACSL to distinguish between the
...
can be enriched with postconditions that belong to one behavior only.
...
they may also be only declared (with a suitable declaration of read
effects) and defined through an axiomatization. 
