...
\exists $\tau$ $x_1$,$\ldots$,$x_n$;
  ...
  && p
\end{lstlisting}
Therefore each quantified variable belongs to a finite interval. Since finite
interval is only computable in practice for integers, this form of quantifier is
limited to \texttt{integer} and its subtype. Thus there is no guarded
quantification over \texttt{float}, \texttt{real}, \C pointers or logic types.
...
\difficultwhy{\lstinline|\\old|}{a deep copy of the location}
...
\difficultwhy{\lstinline|\\assigns|}{the implementation of a memory model}
...
\except{ranges and set comprehensions are limited in order to be finite}
...
\difficultwhy{\lstinline|loop assigns|}{the implementation of a memory model}
...
added within a ghost statement or a default logic label) follows the same rule
than its \acsl counterpart, except that a more restrictive scoping rule must be
respected in addition to the standard \acsl scoping rule: when evaluating
\verb|\at(t,id)| at a propram point $p$, the program point $p'$ denoted by
\verb|id| must be executed after $p$ the program execution flow.
...
\difficultswhy{Hybrid functions and predicates}{the implementation of a memory
  model (or at least to support \lstinline|\\at|)}
...
\difficultswhy{\lstinline|\\base\_addr|, \lstinline|\\block\_length|,
  \lstinline|\\valid| and \lstinline|\\offset|}{the implementation of a memory
  model}
...
\difficultswhy{\lstinline|\\initialized| and \lstinline|\\specified|}{the
