...
\subsection*{\notimplemented{Quantification}}
\eacsl quantification must be computable. They are limited to two limited forms.

\begin{description}
\item[\notimplemented{Guarded integer quantification}] Guarded universal
  quantification is denoted by 
\begin{lstlisting}
...
\item[\notimplemented{Guarded iterator quantification}] In order to iterate over
...
\subsection{Memory locations and \notimplemented{sets of terms}}
\label{sec:locations}
...
bind only one variable. In that way, each \notimplemented{set of terms} is
finite and their members easily identifiable.
\begin{figure}[htbp]
...
\subsection{\notimplemented{Loop annotations}}
\label{sec:loop_annot}

\except{\notimplemented{loop invariants} lose their inductive nature}

Figure~\ref{fig:gram:loops} shows grammar for \notimplemented{loop
  annotations}. There is no syntactic difference with \acsl.
\begin{figure}[htbp]
  \begin{cadre}
...
\subsubsection{\notimplemented{General inductive invariant}}
...
\section{\notimplemented{Sets as first-class values}}
\index{location}
\notimplemented{\nodiff}
