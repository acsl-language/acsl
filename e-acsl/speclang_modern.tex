...
\item[Guarded iterator quantification] In order to iterate over non-integer
  ...
  namely \texttt{nexts} and the \texttt{guards} --- to a type expression. Both
  sets must have the same cardinal. This construct is described by the grammar
  of Figure~\ref{fig:gram:iterator}.
  \begin{figure}[htbp]
    \begin{cadre}
  ...
  Furthermore, the guard of a quantification using an iterator must be a binary
  predicate taking two arguments of the same type than the binder. The first
  argument must be this binder and the second one corresponds to the term from
...
Below, the first and second predicates are invalid while the third one is
valid:
\begin{itemize}
