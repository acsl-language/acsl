...
\except{undefinedness and same lazyness than \C}
...
valid (resp. invalid)\footnote{For \jml, that is only true for a common subset
  to \eacsl and \jml.}. Thus it is possible to reuse tools interpreting \acsl in
order to interpret \eacsl, and it is also possible to perform runtime assertion
checking of \eacsl predicates in the same way than \jml predicates. Reader
interested by the implications (especially issues) of such a choice may read
articles of Patrice Chalin~\cite{chalin05,chalin07}.
...
\item The predicate \lstinline|I| is an invariant, that is if execution of the
  loop body in some state $S$ where the condition \lstinline|c| is true ends
  normally at the end of the body or with a \lstinline{continue} statement,
  \lstinline|I| is true in the resulting state. If the loop condition has side
...
be true at the exit of the loop: if the last ``step'' starts from a state
where \lstinline|I| holds, it performs the side-effects of \lstinline|c|,
...
The only difference with \acsl is that \eacsl does not assume that the predicate
\lstinline|I| holds in the state $S$. In other words a loop invariant
\lstinline|I| is equivalent to put an assertion \lstinline|I| just before
entering the loop and at the very end of the loop body.
...
\lstinline|abrupt| clauses. All the other constructs are unchanged.
