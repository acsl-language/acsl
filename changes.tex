...
\begin{itemize}
  \item More lenient ordering of clauses in contracts (Section~\ref{sec:fn-behavior})
\end{itemize}


\subsection{Version 1.24}
\begin{itemize}
...
\begin{itemize}
  ...
  \item Add \lstinline|alignof| keyword (Section~\ref{sec:sizeof-alignof}).
  \item Move sections on dangling pointers (\ref{sec:dangling_pointers}) and
        well-typed pointers (\ref{sec:welltyped}) in Section~\ref{sec:pointers}.
  \item Move section on initialization (\ref{sec:initialized}) closer to the
        section on pointers (\ref{sec:pointers}).
\end{itemize}
...
(removed in Version 1.22), that was a bit rushed into
the last version
