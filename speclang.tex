...
\begin{example}
  The following definitions \input{signdef.pp} illustrate the
...
A predicate may also be defined by an inductive definition. An
inductive definition of a predicate $P$ has the form
\begin{flushleft}\ttfamily
...
\end{flushleft}
where $P$ occurs only positively in hypothesis $h_1,\ldots,h_l$
...
\emph{axiomatic} definition for a predicate or a logic function, which
amounts to declare the expected profile and a set of axioms.

\begin{todo}
  example TODO
\end{todo}

Like inductive definitions, there is no syntactic conditions which would guarantee axiomatic definitions to be consistent.
