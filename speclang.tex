...
priorite du local naming: inferieur a tout

ambiguite avec ?:


x?y:z:t -> x?(y:z):t ou x?y:(z:t)
...
\begin{example}
  \emph{\texttt{(unsigned char)1000}} is $1000 \bmod 256$ \emph{i.e.} $232$.
\end{example}
...
the loop iteration. For example, it makes sense for a \texttt{do $s$
...
\notimplemented{More generally, a loop invariant may occur anywhere
inside the loop body.}
The meaning is just that the invariant is true at that point,
...
The loop invariant is inductively preserved by the two paths that goes from node inv to itself.
\end{example}
...
higher-order logic.
\begin{description}
\item[Abstraction] The term
  \notimplemented{${\tt \backslash{}lambda}~\tau~x_1,\ldots,x_n ~;~ t$}
...
\begin{remark}{Stouls}
  ...
  Besoin d'invariant sur variables locales surtout si elles sont
  statiques.
...
\begin{verbatim}
  void out_char(char c) {
      ...
      //@ data invariant I : 0 <= col <= 80;

    }
\end{verbatim}

 on devrait les traiter comme des invariants sur globales. Seule
 out\_char devrait preserver cet invariant.
