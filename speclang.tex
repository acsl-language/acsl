...
% [Patrick B.] START proposition:
  ...
  is not necessary, even if the function modifies the formal operand $x$, because
  function calls modify a copy of the effective operands, and 
  the effective operands remain unaltered.
  In fact, $x$ denotes the effective operand of $isqrt$ calls which
  has the same value interpreted in the pre-state than in the
  post-state.
% END proposition
...
\verb|Old|. $\old(e)$\indexttbs{old} is in fact syntactic sugar for $\at(e,Old)$.
...
\item The label \verb|Here| is visible in all statement annotations,
  ...
  all contracts, where it refers to the pre-state for the \texttt{requires}
  clause and the post-state for other clauses. 
% [Patrick B.] START question:
% Est-ce pour permettre l'écriture d'une sorte de ``post assume'' ?
% Si oui, il serait bon de remettre au goût du jour l'exemple fwrite-malloc.c
% Dans ce cas, as-t'on le droit d'utiliser la construction suivante :
% ``\at(Here,\result)'' dans une clause assume (ce n'est jamais dit) ?
% Est-ce la même chose pour la clause ``terminates'' des contrats de
% fonctions ?
% Même question pour la clause ``variant'' des contrats de fonction et
% d'instructions ?
% END question.
It is also visible in data invariants, presented in Section~\ref{sec:invariants}.
% [Patrick B.] START question:
% Et alors, que désigne cette étiquette dans ce genre de propriétés ?
% END question.
...
% terminaisons abruptes. Ce n'est pas dit ici. 
% END remarque
...
Instead of an explicit definition, one may introduce an \emph{axiomatic}
definition, which amounts to declare the expected profile and a set
of axioms.
...
It is up to the user to ensure that the introduction of axioms does
not lead to a logical inconsistency.

Axioms can also be given as \emph{lemmas}, meaning that they are
deducible from others axioms.  Thus, while axioms are always assumed
to be true, a complete verification of an ACSL specification has to
...
Notice that there is no syntactic condition on such inductive
definitions. In essence, an inductive definition of the form $P(args)
\{ \verb|case |c_1:a_1 \ldots \verb|case |c_n:a_n \}$ is just a
shortcut for a declaration of \verb+P+ followed by $n$ axioms
\verb+c_1:a_1,\ldots,c_n:a_n+.  In other words, recursive definitions
are not guaranteed to be consistent, in the same way that axioms may
introduce inconsistency. Of course, tools might provide a way to check
consistency: a standard syntactic criterion for ensuring consistency
of such a definition is to require that each $a_i$ contains exactly
one positive occurrence of $P$ (definite Horn clauses, \url{http://en.wikipedia.org/wiki/Horn_clause}).
