...
\remark{Claude}{Pourquoi le if ternaire a une precedence superieure a l'implication? C'est un choix de Caduceus, que l'on pourrait discuter}
...
Donner des examples, exemples de term unspecified
...
\item The called function returns a state where the properties
  $\old(A_i) \verb|==>| E_i$ hold for each $i$.
\item For each $i$, if the function is called in a pre-state where
  $A_i$ holds, then all memory locations of that
...
~~@*/
\end{flushleft}
Moreover, in some context one might want to guarantee that the
disjunction of \assumes{} clauses is true, meaning informally that all
...
\begin{example}
  ...
  locations is the set of fields $p\verb|->hd|$ for each pointer $p$
  reachable from \verb|root| following \verb|next| fields. See
...
\item The $\texttt{assert for}~id:~p$ variant associates the assertion
  to the name $id$ which must be a behavior identifier for the current
  function.  It means that this assertion is meant to be valid only
  for the considered behavior.
\end{itemize}
...
Remarks:
\begin{itemize}
\item The $\old$ construct is not allowed in loop annotations
\item When a loop exists with \texttt{break} or \texttt{return}, it is
  not required that the loop invariant holds.
\end{itemize}
...
behaviors, limited to \assigns{} and \ensures{} clauses. The notation
for named behaviors is of the form \texttt{behavior for $id$} where
$id$ is the name of some behavior of the current function. Named
statement are meant to be valid only for the corresponding function
behavior, in particular only under the corresponding \assumes{}
clause.
...
TODO: syntax for mutual recursion
