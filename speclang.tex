...
\item The construct $t_1~relop_1~t_2~relop_2~t_3 \cdots t_k$ with
  ...
  However, the $relop_i$ must be in the same ``direction'',
  \emph{i.e.} they must all belong either to $\{\verb|<|, \verb|<=|,
  \verb|==|\}$, $\{\verb|>|,\verb|>=|,\verb|==|\}$, or
  $\{\verb|!=|\}$. In particular, the expression \verb|x < y > z| is
  forbidden. Moreover, \verb|x != y != z| does not mean that \verb|x|
  is different from \verb|z|.
\begin{remark}{Virgile}Dans le dernier cas, on pourrait vouloir avoir x, y
  et z deux à deux distincts, et eventuellement un prédicat n-aire
\verb|\distinct(x,y,z)| comme dans SMT-lib

  Claude: je suis plutot pour refuser les \verb|!=| consecutifs
    oui

  
  il faut aussi detailler  l'utilisation des op de comparaison en
  position de terme. : retourne un entier ou un booleen ? 

  Expliquer: 

   $a < b < c$  est equivalent $a < b \verb|&&| b < c$
   $a < (b < c)$ est equivalent $a \texttt{let} x = b < c ; a < x$
...
\begin{verbatim}
   ...
   \result == a < b : faux car \result == a && a < b

   
   \result == (a < b) : mal type car 'int' == 'bool'

   \result <==> a < b : mal type cat int <==> prop
    
   \result != 0 <==> a < b : OK

   \result == (integer)(a<b) : OK
\end{verbatim}
\end{remark}
...
\begin{oldremark}{Anonyme}Donc on ne peut pas introduire de construction
...
\begin{oldremark}{Claude}Si la conjecture suivante est vraie, alors il
...
\begin{oldremark}{Claude}we need macros \texttt{max\_range} \texttt{min\_range}
...
\begin{oldremark}{All}  syntaxe pour abs et power ? a priori non, on peut
...
\begin{remark}
  ca ne va pas: il faut faire comme la norme IEEE 
...
\begin{remark}{Claude}TODO
Quantification sur (float) et (double) : signifie quantifier sur les
flottants fini. 
\[
\Forall float x ; P <==> \forall real x; isFinite((float)x) ==> P
\]
...
\begin{oldremark}{Patrick}pourquoi seuls les ``behavior'' peuvent avoir un
...
`true', and similarly for \ensures clause. Giving no \assigns clause means
that locations assigned by the function are not specified, so the
caller has no information at all on this function's side effects. 
...
variable within a statement (see~\cite{KR88}, Section \begin{todo}{Claude}x.x
\end{todo}
) 
...
to appear before'' :
partant du principe que toute les propritétes sont relatives a un
point de programe,
$\at(e,id)$ désigne la valeur de $e$ precedent passe au label $id$.

Cela donne la même chose lorsque $id$ est avant le point de programme
...
\begin{oldremark}{Claude}unifier les 2 derniers cas? car a-t-on besoin de
...
\begin{oldremark}{Claude}``logic typedef'' au lieu de ``logic type'' ?
...
\begin{oldremark}{Patrick}on a besoin de ``ghost braces'' pour
