...
\item The construct $t_1~relop_1~t_2~relop_2~t_3 \cdots t_k$ with
  ...
  However, the $relop_i$ must be in the ``same sense'', {\it i.e.} they must
  all belong either to $\{\verb|<|, \verb|<=|, \verb|==|\}$,
  $\{\verb|>|,\verb|>=|,\verb|==|\}$, or $\{\verb|!=|\}$. In
  particular, the expression \verb|x < y > z| is forbidden. Moreover,
\verb|x != y != z| does not mean that \verb|x| is different from \verb|z|.
...
Hexadecimal and octal constants are always non-negative. Suffixes \texttt{u} and
  \texttt{l} are allowed but meaningless.
...
\begin{itemize}
\item Example: (float)0.1 =  (TODO by
  JCF: forme exact avec un mantisse * 2 puissance exposant)

which is approximately $0.100000000000000005551\cdots$
\end{itemize}
...
changer le critere de validite des labels du at: regle de portee
similaire a celle des variables locales. Insister sur la difference
avec le goto.
...
\begin{remark}{Decision du 24 octobre 2007}
\begin{verbatim}
...
in the \reads{} clauses of Section~\ref{sec:logicspec}.
...
A \emph{location} is any set of terms denoting a set of
\emph{modifiable lvalues} as described in Section~\ref{sec:glossary}.
...
\begin{example}
  Here is a longer example, the famouns Dijkstra's Dutch flag algorithm.
  \input{flag.pp}
\end{example}
...
but visible only in the specifications. The grammar is given in
Figure~\ref{fig:gram:ghost}.
...
\item If a non-ghost C function is called in ghost code, it must no
...
\begin{example}
  The following is not valid ghost code:
