...
Conversion of real numbers to float or double values indeed depend on
various possible rounding modes defined by the IEEE754 standard. These
modes are defined by a logic type:
...
\notimplemented{As a consequence, such invariants may occur anywhere
...
\verb|Old|. $\old(e)$ is in fact syntactic sugar for $\at(e,Old)$.
...
Note that no logic labels are visible in global logic declarations
...
may refer to enclosing named behaviors, with the form \texttt{for $id$ :
  \ldots}. Such contracts are only valid for the
...
statement (\texttt{break}, \texttt{continue} or \texttt{return}). For
the \texttt{returns} case, the \result{} construct is allowed and
bound to the value returned (if not a \texttt{void} function).
...
\emph{axiomatized} by a set of axioms.
...
supposed to be deductible from axioms. Thus, while axioms are always
assumed to be true, a complete verification of an ACSL specification
has to provide a proof for each lemma. On the other hand, it is up to
the user to ensure that the introduction of axioms does not lead to a
...
\verb+f(args) = e;+ where \verb+f+ occurs in expression \verb+e+ is
just a shorcut for a declaration of \verb+f+ followed by an axiom
\verb+\forall args; f(args) = e+.  In other words, recursive
definitions are not guaranteed to be consistent, in the same way that
axioms may introduce inconsistency. Of course, tools might provide a
...
higher-order logic.
\begin{description}
\item[Abstraction]\index{lambda@\Lambda}
...
\item[Extended quantifiers]
  ...
  where $quant$ is \notimplemented{\texttt{max}}\index{max@\texttt{max}},
  \notimplemented{\texttt{min}}\index{min@\texttt{min}},
  \notimplemented{\texttt{sum}}\index{sum@\texttt{sum}},
  \notimplemented{\texttt{product}}\index{product@\texttt{product}},
  or \notimplemented{\texttt{numof}}\index{numof@\texttt{numof}},
...
definition. Defined logic types can be  either under
record types\index{record}, or sum types\index{sum}. These definitions may be
recursive.  For record types, the field access notation $t.id$ can be
used, and for sum types, a pattern-matching construction is
available. The grammar rules for these additional constructions are
given in Figure~\ref{fig:gram:logictype}
...
can either be defined with the
...
% NdV: exemple redondant avec le suivant.
% \begin{example}
...
\notimplemented{$\separated(loc_1,..,loc_n)$}\index{separated@\separated}
means that for each
...
annotations. They have the \notimplemented{built-in type
  $loc<A>$}\index{loc@\texttt{loc} type}
...
Note that a clause \verb+assigns l1,..,ln+ is equivalent
...
\begin{itemize}
\item $\fresh(p)$\index{fresh@\fresh},
indicates that $p$ was not allocated in the pre-state.

\item \notimplemented{$\freed(p)$\index{freed@\freed}},
...
An extended syntax of \assigns clauses allows to specify data
dependencies. In the following example, the values assigned to
locations $loc_1,\ldots,loc_n$ can only use the values from locations
$loc'_1,\ldots,loc'_k$:
\begin{flushleft}
...
\item \emph{global} invariants and \emph{type} invariants: the former
  ...
  associated to static types, and may be applied to any variables of
  these types;
\item \emph{strong} invariants and \emph{weak} invariants: strong
  invariants must be valid at any any time during program execution
...
\notimplemented{A \emph{model variable} is a variable introduced in the
  specification with the keyword {\texttt{model}}. Its type must be
  a logic type. Analogously, structures may have \emph{model fields}.
  These are used to provide abstract specifications to functions whose
  concrete implementation must remain private.}
...
is a logic type, not a C type. Also, the semantics above is
closer to the one of B machines.  It has to be noticed that
program verification with model variables does not have a
well-established theoretical background~\cite{marche07,leavens07}, so we
deliberately do not provide a precise semantics in this document .
...
\verb|\initialized| is a predicate taking a set of l-values as
...
\verb|\specified| is a predicate taking a set of l-values as
