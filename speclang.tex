...
various possible rounding modes defined by the IEEE~754 standard. These
...
\item $t_1 .. t_2$ denotes the set of integers between $t_1$ and
  $t_2$, included. If $t_2 > t_1$, this the same as \verb+\empty+
...
\item[Abstraction]\indexttbs{lambda}
  \notimplemented{The term $\bslambda~\tau_1~x_1,\ldots,\tau_n~x_n ~;~ t$}
...
\item[Extended quantifiers]
  ...
  where $quant$ is \notimplemented{\texttt{max}}\indexttbs{max}
  \notimplemented{\texttt{min}}\indexttbs{min}
  \notimplemented{\texttt{sum}}\indexttbs{sum}
  \notimplemented{\texttt{product}}\indexttbs{product}
  or \notimplemented{\texttt{numof}}\indexttbs{numof}
...
\notimplemented{$\separated(loc_1,..,loc_n)$}\indexttbs{separated}
means that for each
