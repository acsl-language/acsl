...
\item All memory locations of the pre-state that do not belong to the
  ...
  post-state. The set $L_1 \cup L_2 \cup \dots$ is interpreted in the
  post-state.
\end{itemize}
...
\indextt{Pre}\indextt{Here}\indextt{Old}
...
\item The label \verb|Here| is visible in all statement annotations,
  ...
  clause and the post-state for other clauses. It is also visible in
  data invariants, presented in Section~\ref{sec:invariants}.
...
or, if the non-termination concerns only a behavior of the function:
\begin{flushleft}\ttfamily
  /*@ behavior always\_terminates: assumes $a_1$; \\
      behavior may\_loop: assumes $a_2$; terminates $p$; \\
  */
\end{flushleft}
...
functions. If the clause is found in a particular behavior, it applies
only to this behavior. Hence, the contract above expresses that if
$a_1$ holds or if $a_2$ and $p$ hold, then the function terminates.
...
As shown above, hybrid predicates and functions should be used
...
Note that a clause \verb+assigns+ $l_1,..,l_n$ is equivalent
