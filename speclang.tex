...
\item Some UTF8 characters may be used in place of some constructs, as
  ...
  \begin{center}
    \begin{tabular}{|l|l|}
      ...
      \verb|!=| & \ensuremath{\not\equiv} \\
      \hline
      \verb|==| & \ensuremath{\equiv} \\
      \hline
      \verb|==>| & \ensuremath{\Longrightarrow} \\
      \hline
      \verb|<==>| & \ensuremath{\Longleftrightarrow} \\
      \hline
      \verb|&&| & \ensuremath{\land} \\
      \hline
      \verb+||+ & \ensuremath{\lor} \\
      \hline
      \verb|\forall| & \ensuremath{\forall} \\
      \hline
      \verb|\exists| & \ensuremath{\exists} \\
      \hline
    \end{tabular}
  \end{center}
\end{itemize}

In this document, we use UTF8 characters to improve readability.
...
\item[Additional connectives] C operators \verb|&&| (UTF8: $\land$),
  \verb+||+ (UTF8: $\lor$) and \verb|!| (UTF8: (??)) are used as logical
...
La syntaxe des cas de pattern est ambigue avec le ou bitabit: 
\verb+\match x ; Nil : y | ... +
...
rajouter des data invariants sur les var locales
...
\begin{remark}{Stouls}
 ...
 \verb|out_char| devrait preserver cet invariant.
...
\begin{example}
  ...
  \input{flag.pp}
  \begin{remark}{Claude}
    rajouter la post permut{Pre,Post}(t,t,n-1)
  \end{remark}
\end{example}
