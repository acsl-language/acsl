...
the cardinal of this type (that is 2 to $8\times\sizeof(t)$). Example:
\begin{itemize}
...
a consistent way as the C code).
...
\begin{itemize}
\item Example: \verb|(float)0.1| = $13421773 \times 2 ^{-27}$
which is approximately $0.100000001490116119\cdots$
\end{itemize}
...
contracts. Grammars for non-terminal \textsl{locations} are given later,
informally, they denote C l-values\index{lvalue}. We also introduce additional constructs
...
\emph{modifiable lvalues} \index{lvalue} as described in
Section~\ref{sec:glossary}.  Only locations are valid as argument to
\assigns{} clauses
