...
\begin{remark}{DP}
  Parler aussi des real (et la difference avec float et double)
...
\begin{remark}{PC}
 ...
 contraintes sur les casts :


  que veut dire (char)1000 dans la logique ?? 


  les casts implicites vers les types du C pourrait etre interdits, et
  les casts explicites operent une truncature.


  logic int f() { 0 } -> cast implicite ! erreur !

  logic integer f() { 0 } -> OK
...
Detailler, en particulier les bit-a-bit sur les entiers mathematiques
...
\begin{remark}{Patrick}
 ...
 veut des $\old$ partout: design toujours le pre-state de la function. 
...
\begin{verbatim}
  ...
  MAIS remarque de Patrick: pourquoi ne pas parler de la disjonction des assumes ?
\end{verbatim}
\end{remark}
...
et de toute facon il y a besoib de critere de separation qui assure
