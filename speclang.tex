...
correspond more or less to pure C expressions, without C function
calls (but logic function calls are allowed), with additional constructs
...
The language of logic expressions is typed. Logic types are:
\begin{itemize}
...
\subsection{About arithmetic overflow}
...
\]
\[
\frac{\Gamma\vdash e_1:\tau_1,b_1 \quad e_2:\tau_2,b_2}{\vdash e_1,e_2: ???}
\]
\[
...
We could allow to pose a loop invariant not only at the beginning of
the loop iteration. For example, it could make sense for a \texttt{do
  $s$ while $(c)$;} loop to pose a loop invariant just after the loop
body $s$.

\begin{example}
  In the following, the loop invariant is correct, but it would not be correct if placed at the beginning of the loop.
  \input{dowhile.pp}
\end{example}


More generally, a loop invariant could occur anywhere inside the loop
...
inductive: is preserved by a loop iteration. It even possible to allow
several loop invariants at different places in the loop body. These
advanced extensions could be useful for complex control flows, in
...
Shortcuts:
\begin{itemize}
\item $\verb|\null|$ is an extra notation for the NULL pointer. It is
  a shorcut to \verb|(void*)0| \footnote{or to \texttt{NULL}, if
