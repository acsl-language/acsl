...
defined as follows:
\begin{itemize}
\item ``mathematical'' types: \verb|integer|\indextt{integer} for
  unbounded, mathematical integers, \verb|real|\indextt{real} for
  real numbers, \verb|boolean|\indextt{boolean} for booleans (with
...
\texttt{sizeof}\index{sizeof} operator may be used in annotations and is
...
\begin{flushleft}\ttfamily
predicate \verb|\is_NaN|\indexttbs{is\_NaN}(float x); \\
predicate \verb|\is_NaN|\indexttbs{is\_NaN}(double x); \\
predicate \verb|\is_finite|\indexttbs{is\_finite}(float x); \\
predicate \verb|\is_finite|\indexttbs{is\_finite}(double x);
\end{flushleft}
...
\begin{itemize}
\item $\old(e)$\indexttbs{old} denotes the value of predicate or term $e$ in the
...
% [Patrick B.] START proposition:
  ...
  invariant for the locations that are not member of $L$.
% END proposition:
\end{itemize}

A loop annotation preceded by
$\texttt{for}~id_1,\ldots,id_k\texttt{:}$ is similar as above,
although applies only for behaviors $id_1,\ldots,id_k$, hence in
...
\subsection{Memory footprint specification: \texttt{reads} clause}

\begin{figure}[t]
  \fbox{\begin{minipage}{0.97\linewidth}
      \input{logicreads.bnf}
    \end{minipage}}
  \caption{Grammar for logic declarations with \texttt{reads} clauses}
...
\begin{example}
  ...
  a \texttt{reads} clause.  \input{nb_occ_reads.pp} If for example a
  piece of code between labels $L_1$ and $L_2$ modifies $t[k]$ for
  some index $k$ outside $i..j$, then one can deduce that
  $\verb|nb_occ|\{L_1\}(t,i,j,e)=\verb|nb_occ|\{L_2\}(t,i,j,e)$.
